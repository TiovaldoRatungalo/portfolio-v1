...
const StrokeText = ({
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      // Dispatch event when animation fully completes so other components can react
      tl.call(() => {
        window.dispatchEvent(new CustomEvent('stroketext:complete'));
      });
  ...
  return (
    <span
      ...
      style={{ ...style, '--stroke-text-height': `${Math.round(fontSize * 1.3)}px`, '--stroke-dash': dash }}
      role="img"
